...
class Index extends Component {
    constructor(props) {
        ...
        this.getPedidosList = this.getPedidosList.bind(this);
        ...
        let markers = this.state.calendarEvents;
    }
    componentDidMount = () => {
        this.getPedidosList();
    }
    pushArrayEvent = (title,fecha) => {
        var newEvent = {};
            newEvent = {  title: title, date: fecha };

          this.setState({calendarEvents: DefaultEvents.concat(newEvent)});
    }
      ...
      let  pedidosRef = ref.on("child_added", function(usuario) {
      ref.child(`${usuario.key}/pedidos`).on("child_added", function (itemcol) {
            var fecha = '';
            var total = '';
            var newEvent = {};
            itemcol.forEach(function (doc) {
              if(doc.key === 'total' && doc.val() > 0){
                total = 'Ventas: $' + doc.val()
                console.log(doc.key,total)
                console.log('fecha',fecha)
                newEvent = {  title: total, date: fecha,   className: 'bg-success text-white' }
                this.setState({
                    calendarEvents: this.state.calendarEvents.concat(newEvent),
                    selectedDay: null
                })
              }
              if(doc.key === 'fechaPedido') {
                fecha = doc.val()
              }
            }.bind(this));
        }.bind(this)); // pedido
    ...
    renderRedirect = () => {
      if (this.state.redirect) {
        return <Redirect to='/ecommerce-orders' />
      }
    }
